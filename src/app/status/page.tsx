import type { ApiResponse, HealthData } from "@/src/types.ts";
import type { Metadata } from "next";

import { getTranslations } from "next-intl/server";

import { Table, TableBody, TableCell, TableHead, TableHeaderCell, TableRow } from "@/src/components/table/Table.tsx";
import { Card } from "@/src/components/ui/Card.tsx";
import { CardHeadline } from "@/src/components/ui/CardHeadline.tsx";
import { appConfig } from "@/src/config/index.ts";

export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations("Status");
  return {
    title: t("heading"),
  };
}

async function fetchHealth(): Promise<HealthData | null> {
  try {
    const res = await fetch(`${appConfig.app.url}/api/health`, { cache: "no-store" });
    const json: ApiResponse<HealthData, HealthData> = await res.json();
    return json.success ? json.data : (json.details ?? null);
  } catch {
    return null;
  }
}

export default async function Page() {
  const t = await getTranslations("Status");
  const health = await fetchHealth();

  return (
    <Card>
      <CardHeadline
        title={t("heading")}
        icon={
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 640 640"
            fill="currentColor"
            className="size-6 text-neutral-600"
            aria-hidden="true"
          >
            <path d="M160 96C124.7 96 96 124.7 96 160L96 224C96 259.3 124.7 288 160 288L480 288C515.3 288 544 259.3 544 224L544 160C544 124.7 515.3 96 480 96L160 96zM376 168C389.3 168 400 178.7 400 192C400 205.3 389.3 216 376 216C362.7 216 352 205.3 352 192C352 178.7 362.7 168 376 168zM432 192C432 178.7 442.7 168 456 168C469.3 168 480 178.7 480 192C480 205.3 469.3 216 456 216C442.7 216 432 205.3 432 192zM160 352C124.7 352 96 380.7 96 416L96 480C96 515.3 124.7 544 160 544L480 544C515.3 544 544 515.3 544 480L544 416C544 380.7 515.3 352 480 352L160 352zM376 424C389.3 424 400 434.7 400 448C400 461.3 389.3 472 376 472C362.7 472 352 461.3 352 448C352 434.7 362.7 424 376 424zM432 448C432 434.7 442.7 424 456 424C469.3 424 480 434.7 480 448C480 461.3 469.3 472 456 472C442.7 472 432 461.3 432 448z" />
          </svg>
        }
        iconPosition="right"
      />
      {health ? (
        <Table>
          <TableHead>
            <TableRow>
              <TableHeaderCell>{t("property")}</TableHeaderCell>
              <TableHeaderCell>{t("value")}</TableHeaderCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {Object.entries(health).map(([key, value]) => (
              <TableRow key={key}>
                <TableCell className="font-medium">{key}</TableCell>
                <TableCell>{String(value)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
      ) : (
        <p>{t("fetchError")}</p>
      )}
    </Card>
  );
}
